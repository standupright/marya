...
const SEARCH_BAR_FIXED = '.search-bar--fixed';
const SHIFT = header.offsetHeight;
...
const SEARCH_BAR_ACTIVE = 'search-bar--active';
const SEARCH_BAR_OPENED = 'search-bar--opened';
const SEARCH_ACTIVE = 'controls-list__icon-wrapper--active';
...
if (search) {
  search.addEventListener('click', () => {
    if (searchBar.classList.contains(SEARCH_BAR_ACTIVE)) {
      searchBar.classList.remove(SEARCH_BAR_ACTIVE);
      searchIcon.classList.remove(SEARCH_ACTIVE);
    } else {
      searchBar.classList.add(SEARCH_BAR_ACTIVE);
      searchIcon.classList.add(SEARCH_ACTIVE);
    }
  });
}
...
if (burgerMenuToggle) {
  burgerMenuToggle.addEventListener('click', () => {
    if (burgerMenu.classList.contains(BURGER_MENU_OPENED)) {
      ...
      searchBar.classList.remove(SEARCH_BAR_OPENED);
      ...
      searchBar.classList.add(SEARCH_BAR_OPENED);
